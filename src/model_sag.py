...
from models.sag import SNRAdaptiveGating
...
class AWN_SAG(nn.Module):
    def __init__(self,
        ...
        for i in range(self.num_levels):
            self.levels.add_module(
                'level_' + str(i),
                LevelTWaveNet(self.in_channels, kernel_size,
        ...
        # SAG modülü — SE attention'dan sonra geliyor
        self.sag = SNRAdaptiveGating(n_subbands=self.out_channels)
    ...
    def forward(self, x, snr):
        ...
        # SE attention (orijinal)
        x = torch.mul(self.SE_attention_score(x), x)

        # SAG — SNR bilgisiyle gating
        snr = snr.view(-1, 1).float()
        x = self.sag(x, snr)
